...
from game_screen.screen_object import WIN
...
class Object(pygame.sprite.Sprite):
    ...
    def health_bar(self, pos_x, pos_y, width, height):
        # max health bar
        pygame.draw.rect(
            self.window, 
            (255, 0, 0), 
        ...
        # cur health bar
        cur_health = self.obj.get("cur_health")
        max_health = self.obj.get("max_health")
        ...
        pygame.draw.rect(
            self.window, 
            (0, 128, 0),
            (pos_x, pos_y, int(self.width * (cur_health / max_health)), height)
        )
